...
namespace T1.B1.MetaData
{
    ...
    {
        static private readonly string dataTipEco = "HCO_FRP0007";
        static private readonly string dataCont = "HCO_FRP0006";
        static private readonly string dataMuni = "HCO_FRP0005";
        static private readonly string dataDpto = "HCO_FRP0004";
        static private readonly string dataTipDoc = "HCO_FRP0003";
            ...
            try
            {
                ...
                for (int i = 0; i < intTotal; i++)
                {
                    if (T1.B1.MainObject.Instance.B1Company.GetXMLobjectType(strPath, i) == SAPbobsCOM.BoObjectTypes.oUserTables)
                    {
                        ...
                        try
                        {
                            objUTMD = (UserTablesMD)T1.B1.MainObject.Instance.B1Company.GetBusinessObjectFromXML(strPath, i);
                    ...
                    else if (T1.B1.MainObject.Instance.B1Company.GetXMLobjectType(strPath, i) == SAPbobsCOM.BoObjectTypes.oUserFields)
                    {
                        ...
                        try
                        {
                            objUFMD = (UserFieldsMD)T1.B1.MainObject.Instance.B1Company.GetBusinessObjectFromXML(strPath, i);
                    ...
                    else if (T1.B1.MainObject.Instance.B1Company.GetXMLobjectType(strPath, i) == SAPbobsCOM.BoObjectTypes.oUserObjectsMD)
                    {
                        ...
                        try
                        {
                            ...
                            objUOMD = (UserObjectsMD)T1.B1.MainObject.Instance.B1Company.GetBusinessObjectFromXML(strPath, i);
                            ...
                            else
                            {
                                if (Settings._Main.updateUDOForm)
                                {
                                    ...
                                    objUOMD = (UserObjectsMD)T1.B1.MainObject.Instance.B1Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.oUserObjectsMD);
        ...
        static private bool isTableCreated(string TableName)
        {
            ...
            try
            {
                objRecord = (Recordset)T1.B1.MainObject.Instance.B1Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset);
                ...
                {
                    int intCount = int.Parse(objRecord.Fields.Item(0).Value.ToString());
                    if (intCount == 1)
                    {
                        blResult = true;
                    }
        ...
        static public void loadMuni()
        {
            ...
            try
            {
                ...
                UDOService = objCompanyService.GetGeneralService(dataMuni);
                headerInfo = (GeneralData)UDOService.GetDataInterface(SAPbobsCOM.GeneralServiceDataInterfaces.gsGeneralData);
                ...
                {
                    using (StreamReader sr = new StreamReader(AppDomain.CurrentDomain.BaseDirectory + "LoadData\\" + dataMuni))
                    {
                        while (!sr.EndOfStream)
                        {
                            ...
                            headerInfo.SetProperty("U_Departamento", strResult[2].Trim());
        ...
        static public void loadDepto()
        {
            ...
            try
            {
                ...
                objCompanyService = MainObject.Instance.B1Company.GetCompanyService();
                UDOService = objCompanyService.GetGeneralService(dataDpto);
                headerInfo = (GeneralData)UDOService.GetDataInterface(SAPbobsCOM.GeneralServiceDataInterfaces.gsGeneralData);
                try
                {
                    objList = UDOService.GetList();
                ...
                if (objList == null || objList.Count == 0)
                {
                    using (StreamReader sr = new StreamReader(AppDomain.CurrentDomain.BaseDirectory + "LoadData\\" + dataDpto))
                    {
                        while (!sr.EndOfStream)
                        {
                            string strLine = sr.ReadLine();
        ...
        static public void loadTipContrib()
        {
            ...
            try
            {
                ...
                objCompanyService = MainObject.Instance.B1Company.GetCompanyService();
                UDOService = objCompanyService.GetGeneralService(dataCont);
                headerInfo = (GeneralData)UDOService.GetDataInterface(SAPbobsCOM.GeneralServiceDataInterfaces.gsGeneralData);
                try
                {
                    objList = UDOService.GetList();
                ...
                if (objList == null || objList.Count == 0)
                {
                    using (StreamReader sr = new StreamReader(AppDomain.CurrentDomain.BaseDirectory + "LoadData\\" + dataCont))
                    {
                        while (!sr.EndOfStream)
                        {
                            string strLine = sr.ReadLine();
        ...
        static public void loadActivEcon()
        {
            ...
            try
            {
                ...
                objCompanyService = MainObject.Instance.B1Company.GetCompanyService();
                UDOService = objCompanyService.GetGeneralService(dataTipEco);
                headerInfo = (GeneralData)UDOService.GetDataInterface(SAPbobsCOM.GeneralServiceDataInterfaces.gsGeneralData);
                try
                {
                    objList = UDOService.GetList();
                ...
                if (objList == null || objList.Count == 0)
                {
                    using (StreamReader sr = new StreamReader(AppDomain.CurrentDomain.BaseDirectory + "LoadData\\" + dataTipEco))
                    {
                        while (!sr.EndOfStream)
                        {
                            string strLine = sr.ReadLine();
        ...
        static public void loadTipDoc()
        {
            ...
            try
            {
                ...
                objCompanyService = MainObject.Instance.B1Company.GetCompanyService();
                UDOService = objCompanyService.GetGeneralService(dataTipDoc);
                headerInfo = (GeneralData)UDOService.GetDataInterface(SAPbobsCOM.GeneralServiceDataInterfaces.gsGeneralData);
                ...
                if (objList == null || objList.Count == 0)
                {
                    using (StreamReader sr = new StreamReader(AppDomain.CurrentDomain.BaseDirectory + "LoadData\\" + dataTipDoc))
                    {
                        while (!sr.EndOfStream)
                        {
                            string strLine = sr.ReadLine();
                            ...
                            using (StreamReader sr = new StreamReader(AppDomain.CurrentDomain.BaseDirectory + strFilePath))
                            {
                                while (!sr.EndOfStream)
                                {
                                    ...
                                    for (int i = 0; i < strResult.Length; i++)
                                    {
                                        ...
                                        if (i == 0)
                                        {
                                            oResult = (GeneralDataParams)UDOService.GetDataInterface(SAPbobsCOM.GeneralServiceDataInterfaces.gsGeneralDataParams);
                                            strCode = strResult[i];
