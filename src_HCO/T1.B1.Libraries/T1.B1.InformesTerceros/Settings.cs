...
namespace T1.B1.InformesTerceros
{
    public class Settings
    {
        ...
        public class BalanceTerceros : Westwind.Utilities.Configuration.AppConfiguration
        {
            ...
            {
                reportType = "BALTER";
                getObjectControl = "select Code, Name, DocEntry, U_TransType, U_LastDate, U_LastTrans from[@HCO_T1ITR100] where U_LastDate < convert(datetime,'[--LastDate--]',112)";
                getTransactionList = "select distinct TransId from OJDT where TransId > coalesce((select distinct U_LastTrans from [@HCO_T1ITR100]),-1) order by TransId ASC";
                upsertObjectControl = "MERGE [@HCO_T1ITR100] as T0 "+
                    ...
                    " when not matched then insert([Code], [Name], [DocEntry], [U_LastDate], [U_LastTrans], [U_ReportType] ) values(newid(), newid(), (coalesce((select max(DocEntry) from [@HCO_T1ITR100]),0)+1),getdate(),[--LastTrans--],'" + reportType + "');";
                UpsertCurrentBalance = "MERGE " +
                    ...
                    " values (newid(),newid(),(coalesce((select max(DocEntry) from [@HCO_T1ITR101]),0)+1),S1.[U_Account]" +
