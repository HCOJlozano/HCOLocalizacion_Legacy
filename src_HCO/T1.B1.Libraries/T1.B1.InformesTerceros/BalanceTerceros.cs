...
namespace T1.B1.InformesTerceros
{
    class BalanceTerceros
    {
        ...
        public static void exploteJEInformation(int JournalEntryNumber)
        {
            ...
            try
            {
                objJE = (JournalEntries)MainObject.Instance.B1Company.GetBusinessObject(BoObjectTypes.oJournalEntries);
                if(objJE.GetByKey(JournalEntryNumber))
                {
                    objBPDict = null; // T1.B1.RelatedParties.Instance.getBPThirdPartyRelation();
                    for (int i = 0; i < objJE.Lines.Count; i++)
                    {
                        ...
                        try
                        {
                            if (!String.IsNullOrEmpty(objJE.Lines.UserFields.Fields.Item("U_HCO_RELPAR").Value.ToString()))
                            {
                                strThirdParty = objJE.Lines.UserFields.Fields.Item("U_HCO_RELPAR").Value.ToString();
                            }
                        }
                        catch (Exception er)
                        {
                            _Logger.Error("The UDF U_HCO_RELPAR was not found in DB");
                        }
                        #endregion
                    ...
                    foreach (string strCommand in cmdList)
                    {
                        try
                        {
                            objRs = (Recordset)MainObject.Instance.B1Company.GetBusinessObject(BoObjectTypes.BoRecordset);
        ...
        public static List<int> getTransactionList()
        {
            ...
            try
            {
                ...
                objRSControl = (Recordset)MainObject.Instance.B1Company.GetBusinessObject(BoObjectTypes.BoRecordset);
                if(objRSControl != null)
                {
                    ...
                    if(objRSControl.RecordCount > 0)
                    {
                        ...
                        {
                            int intTransId = int.Parse(objRSControl.Fields.Item(0).Value.ToString());
